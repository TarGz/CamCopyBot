...
class CamCopyBot():
	...
	def backupVideo(_self,source):
		...
		try:
			...
			if(file_extension == ".MP4"):
				_self.po.send_message("backup done for the file " + dest, title="Backup done:"+dest,sound="bike")
	...
	def checkForVideoInPath(self):
		...
		while True:
			self.camera_files = glob.glob(self.camera_path+"*.*")
			self.storage_files = glob.glob(self.storage_path+"*.*")
			# spinner.next()
			
			# print(self.camera_files);
			# print(self.storage_files);
