...
class CamCopyBot():
	...
	def backupVideo(_self,source):
		...
		if(os.path.isfile(file_allready_there)):
			print("file allready exist deleting it: "+file_allready_there)
			...
			if(len(self.camera_files) > 0):
				...
				try: 
					...
					for entry in self.camera_files:
						...
						print(backup_list);
					self.po.send_message(backup_list, title="Backuping batch",sound="bike")
					backup_list = ""
				except NameError:
					e = sys.exc_info()
					print("pushover fail")
			...
			#####################
				...
				try: 
					...
					for entry in self.storage_files:
						...
						print(upload_list);
					self.po.send_message(upload_list, title="Backuping batch",sound="bike")
					upload_list = ""
				except NameError:
					e = sys.exc_info()
					print("pushover fail")
